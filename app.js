// ============================================
// YOUTUBE FULL INFO EXTRACTOR - LẤY MỌI THÔNG TIN (BẢN BẢO MẬT)
// ============================================
...
// ============================================
// 1. HỆ THỐNG ĐĂNG NHẬP & BẢO MẬT (TỪ FILE 2)
// ============================================
    ...
    modal.innerHTML = `
        ...
        ">
            <h2 style="color:#ff0000; margin-bottom:10px;">
                <i class="fab fa-youtube"></i> ĐĂNG NHẬP
            </h2>
            <p style="margin-bottom:20px; color:#666;">
                Vui lòng đăng nhập để sử dụng công cụ.<br>
                <small>(Tài khoản mới được tặng 3 ngày dùng thử)</small>
            </p>
            ...
            <div class="auth-btns">
                <button onclick="handleLogin()" class="btn-primary" 
                        style="width:100%; margin-bottom:10px; padding:12px; background:#ff0000; color:white; border:none; border-radius:8px; cursor:pointer;">
                    ĐĂNG NHẬP
                </button>
                <button onclick="handleRegister()" 
                        style="width:100%; background:#666; color:white; border:none; padding:12px; border-radius:8px; cursor:pointer;">
                    ĐĂNG KÝ MỚI
                </button>
            </div>
            <div id="auth-msg" style="margin-top:15px; color:red; font-weight:bold;"></div>
            <div style="margin-top:20px; font-size:12px; color:#999;">
                <p>Hoặc bạn có thể dùng API Key trực tiếp (không cần đăng nhập):</p>
                ...
                <button onclick="useApiKeyDirectly()" 
                        style="margin-top:10px; padding:8px 20px; background:#3498db; color:white; border:none; border-radius:4px; cursor:pointer;">
                    Dùng API Key
                </button>
            </div>
        </div>
...
async function handleRegister() {
    ...
    try {
        ...
        alert("❌ Lỗi kết nối Backend! Vui lòng thử lại sau.");
    }
}
...
async function handleLogin() {
    ...
    try {
        ...
        document.getElementById('auth-msg').innerText = "❌ Lỗi kết nối Backend!";
    }
}

function useApiKeyDirectly() {
    ...
    // Hiển thị ô API Key trong giao diện chính
    ...
    resultDiv.innerHTML = `
        ...
        ">
            <h2 style="color:#856404;">
                <i class="fas fa-crown" style="color:#ffd700;"></i> TÀI KHOẢN HẾT HẠN
            </h2>
            <p style="font-size: 18px; margin: 20px 0; color:#856404;">${msg}</p>
            ...
            <div style="display: flex; gap: 20px; justify-content: center; margin: 30px 0; flex-wrap: wrap;">
                ...
                " onmouseover="this.style.transform='translateY(-5px)'" onmouseout="this.style.transform='translateY(0)'">
                    ...
                    <button onclick="selectPackage('monthly')" style="
                        margin-top: 15px;
                        padding: 10px 20px;
                        background: #ff6b6b;
                        color: white;
                ...
                " onmouseover="this.style.transform='translateY(-5px)'" onmouseout="this.style.transform='translateY(0)'">
                    ...
                    <button onclick="selectPackage('yearly')" style="
                        margin-top: 15px;
                        padding: 10px 20px;
                        background: #4ecdc4;
                        color: white;
            ...
            ">
                <h4 style="color:#667eea; margin-bottom:10px;">
                    <i class="fas fa-credit-card"></i> HƯỚNG DẪN THANH TOÁN
                </h4>
                <p style="text-align: left; margin: 10px 0;">
                    1. <strong>Chuyển khoản ngân hàng:</strong>
                </p>
                <div style="
                    background: #f8f9fa;
                    padding: 15px;
                ...
                ">
                    ...
                    <p>📝 <strong>Số tài khoản:</strong> <code style="background:#e9ecef; padding:2px 6px; border-radius:4px;">123456789</code></p>
                    <p>👤 <strong>Chủ tài khoản:</strong> NGUYEN VAN A</p>
                    <p>💬 <strong>Nội dung chuyển khoản:</strong> <code style="background:#e9ecef; padding:2px 6px; border-radius:4px;">Gia han ${currentUser.username} ${currentUser.userId || ''}</code></p>
                </div>
                
                <p style="text-align: left; margin: 10px 0;">
                    2. <strong>Sau khi chuyển khoản:</strong>
                </p>
                <ul style="text-align: left; padding-left: 20px; color:#666;">
                    <li>Chụp ảnh biên lai chuyển khoản</li>
                    <li>Gửi ảnh qua Zalo: 0123.456.789</li>
                    <li>Hoặc gửi email đến: support@youtube-extractor.com</li>
                    <li>Tài khoản sẽ được kích hoạt trong vòng 15 phút</li>
                </ul>
            </div>
...
function selectPackage(packageType) {
    alert(`Bạn đã chọn gói ${packageType === 'monthly' ? 'tháng' : 'năm'}. Vui lòng thực hiện chuyển khoản theo hướng dẫn.`);
}

// ============================================
// 2. HÀM XỬ LÝ URL & VIDEO ID
// ============================================
...
// ============================================
// 4. HÀM LẤY TẤT CẢ THÔNG TIN (CẬP NHẬT CHO BACKEND)
// ============================================
...
async function fetchChannelInfo(channelId, apiKey) {
    try {
        // Nếu dùng backend, không lấy thông tin kênh
        if (currentUser) {
            return null;
        }
        // Nếu dùng API Key trực tiếp
...
async function getFullVideoInfo() {
    ...
    // Nếu dùng API Key trực tiếp (không đăng nhập)
    if (!currentUser) {
        const apiKey = apiKeyInput.value.trim();
        if (!apiKey) {
            alert('🔑 Vui lòng nhập API Key của bạn hoặc đăng nhập để sử dụng');
            return;
        }
    }
    ...
    try {
        ...
        const apiKey = currentUser ? '' : apiKeyInput.value.trim();
        ...
        if (videoData.snippet.channelId && !currentUser) {
        ...
        // Kiểm tra nếu lỗi là hết hạn tài khoản
        if (error.message.startsWith("EXPIRED:")) {
            showPaymentUI(error.message.replace("EXPIRED:", ""));
        } else {
            resultDiv.innerHTML = `
                <div class="error">
                    ...
                    <div class="error-details">
                        ...
                        <ul>
                            <li>${currentUser ? 'Tài khoản hết hạn hoặc gặp sự cố' : 'API Key không hợp lệ hoặc đã hết hạn'}</li>
                        ...
                        <ol>
                            ${currentUser ? 
                                '<li>Gia hạn tài khoản để tiếp tục sử dụng</li>' : 
...
youtubeUrlInput.addEventListener('paste', (e) => {
    setTimeout(() => {
        if (currentUser || apiKeyInput.value) {
            getFullVideoInfo();
        }
    }, 500);
});
...
console.log('🎬 YouTube Full Info Extractor v3.0 (Bản bảo mật)');
console.log('📊 Có thể lấy 25+ loại thông tin khác nhau');
console.log('👉 Hướng dẫn: Đăng nhập hoặc dùng API Key → Dán URL → Click LẤY THÔNG TIN');

// Kiểm tra và ẩn/hiện ô API Key dựa trên trạng thái đăng nhập
function checkAuthStatus() {
    ...
    if (token && userData) {
        ...
        // Hiển thị ô API Key nếu chưa đăng nhập
        const apiKeyContainer = apiKeyInput.parentElement;
...
checkAuthStatus();
// ============================================
// BỔ SUNG: LOGIC XUẤT FILE & NÚT PLAN
// ============================================

// 1. Gán sự kiện cho các nút xuất file trong HTML
document.getElementById('exportJson')?.addEventListener('click', () => {
    if (!fullVideoData) return alert("Vui lòng lấy thông tin video trước!");
    downloadRawData(); // Hàm này đã có sẵn trong code cũ của bạn
});
...
document.getElementById('exportCSV')?.addEventListener('click', () => {
    ...
    const rows = [
        ["Tieu de", fullVideoData.snippet.title],
        ["Video ID", fullVideoData.id],
        ["Kenh", fullVideoData.snippet.channelTitle],
        ["Luot xem", fullVideoData.statistics.viewCount],
        ["Luot thich", fullVideoData.statistics.likeCount],
        ["Binh luan", fullVideoData.statistics.commentCount],
        ["Link", `https://www.youtube.com/watch?v=${fullVideoData.id}`]
    ];
    ...
    link.setAttribute("download", `youtube_info_${fullVideoData.id}.csv`);
...
document.getElementById('exportPDF')?.addEventListener('click', () => {
    ...
    doc.setFontSize(18);
    doc.text("YOUTUBE VIDEO REPORT", 10, 20);
    
    doc.setFontSize(12);
    doc.text(`Tieu de: ${fullVideoData.snippet.title.substring(0, 50)}...`, 10, 40);
    doc.text(`Kenh: ${fullVideoData.snippet.channelTitle}`, 10, 50);
    doc.text(`Luot xem: ${fullVideoData.statistics.viewCount}`, 10, 60);
    doc.text(`Luot thich: ${fullVideoData.statistics.likeCount}`, 10, 70);
    doc.text(`ID: ${fullVideoData.id}`, 10, 80);
    
    doc.save(`youtube_report_${fullVideoData.id}.pdf`);
});

// 2. Hàm mở bảng chọn gói (Pricing Plan)
function openPricingModal() {
    showPaymentUI("Chọn gói nâng cấp để không bị giới hạn tính năng");
}

// 3. Cập nhật hàm updateUIForLoggedInUser để hiện nút Plan
// (Ghi đè hoặc thêm vào phần tạo nút logout trong app.js)
const originalUpdateUI = updateUIForLoggedInUser;
updateUIForLoggedInUser = function() {
    originalUpdateUI();
    
    const header = document.querySelector('.header');
    if (header && !document.querySelector('.btn-plan')) {
        const planBtn = document.createElement('button');
        planBtn.className = "btn-plan";
        planBtn.innerHTML = `<i class="fas fa-crown"></i> Nâng cấp Plan`;
        ...
        `;
        planBtn.onclick = openPricingModal;
